...
# Run development server (starts PHP server, queue, logs, and Vite)
composer run dev
...
### Admin Panel
- Filament 4 admin panel at `/admin` with login
- Panel configured in `app/Providers/Filament/AdminPanelProvider.php`
- Resources auto-discovered from `app/Filament/Resources`
- Primary color: Amber

### Database
- MariaDB database
- Standard Laravel tables (users, sessions, jobs, cache)
- Shares database with legacy applications (many wp_* and other tables exist)

### Key Directories
- `app/Ldap/` - LDAP models and repository
- `app/Filament/` - Filament resources, pages, widgets
