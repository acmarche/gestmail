...
// Was 'ldap_simple' until the LDAP -> SQL migration; passwords now live in the
// MariaDB citizen.citoyens table, which is what Dovecot authenticates against.
...
// Must stay in sync with App\Support\DovecotPassword, which writes the same
// scheme from the GestMail side.
...
// 32 MiB rather than PHP's 64 MiB default: Dovecot re-verifies this hash on every
// IMAP/POP/SMTP login. Keep identical to DovecotPassword::OPTIONS.
...
// REQUIRED: the sql driver calls hash_password() with no explicit method, so this
// value also decides whether a prefix is written at all (see password.php, the
// `$prefixed = $rcmail->config->get('password_algorithm_prefix')` line). Left empty,
// Roundcube stores a bare hash, Dovecot falls back to default_pass_scheme = SSHA,
// and every changed password silently stops working.
...
// The citizen database, not Roundcube's own -- leave empty and the driver would
// use db_dsnw (roundcubewebmail), where the citoyens table does not exist.
...
//
// Mirrors App\Ldap\CitoyenHandler::changePassword(): write the new {ARGON2ID} hash,
// drop the migrated LDAP hash so it can no longer be used to log in, and stamp the
// change. Clearing legacy_password is not optional -- Dovecot's password_query falls
// back to it whenever userPassword is empty.
//
// The username_normalize plugin strips @marche.be at login, so %u is normally the bare
// uid; matching mail as well keeps the query correct if that ever changes. Both columns
// are UNIQUE, so at most one row is touched.
$config['password_query'] = 'UPDATE citoyens'
    . ' SET userPassword = %P, legacy_password = NULL, password_changed_at = NOW()'
